 ...
 *
 * Teachers pick a Surah by name instead of typing its number (FR5). The data model of
 * Figure 4.7 is untouched — surah_from / surah_to are still the numeric columns required
 * by first normal form (§4.7); only the input control and the report labels changed.
 * The ayah count is what bounds the Ayah fields server-side.
 */
final class Surah
{
    ...
    public static function exists(?int $n): bool
    {
        return $n !== null && isset(self::ALL[$n]);
    }

    public static function name(?int $n, string $locale = 'ar'): string
    {
        if (!self::exists($n)) {
            return (string) $n;
        }

        return $locale === 'ar' ? self::ALL[$n][0] : self::ALL[$n][1];
    }

    public static function ayahCount(?int $n): int
    {
        return self::exists($n) ? self::ALL[$n][2] : 286;
    }

    /** Human-readable recitation range, e.g. "البقرة 1 – 5" — never a bare "2:1 – 2:5". */
    public static function range(?int $sFrom, ?int $aFrom, ?int $sTo, ?int $aTo, string $locale = 'ar'): string
    {
