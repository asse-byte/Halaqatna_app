  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6" data-testid="badges-grid">
        {d?.all.map((b, i) => { const at = earned.get(b.badge_id); return (
          <div key={b.badge_id} data-testid={`badge-${b.badge_id}`} className={`glass rounded-2xl p-4 text-center animate-rise ${at ? "" : "opacity-50 grayscale"}`} style={{ animationDelay: `${i * 40}ms` }}>
            ...
            <div className="text-[11px] text-muted-foreground">{at ? `${t("earned_at")} ${String(at).slice(0, 10)}` : `${b.condition_type} ≥ ${b.condition_value}`}</div>
          </div>); })}
      </div>
      ...
      <div className="glass max-w-xl divide-y divide-border/60 rounded-2xl" data-testid="xp-ledger">
        {xp?.ledger.slice(0, 20).map((e) => <div key={e.entry_id} className="flex items-center justify-between px-4 py-2 text-sm"><span>{t(`reason_${e.reason}`)}<span className="ms-2 font-mono text-[11px] text-muted-foreground" dir="ltr">{String(e.created_at).slice(0, 10)}</span></span><span className="font-mono font-semibold text-gold">+{e.points}</span></div>)}
      </div>
    </div>
  );
}
